...
Int_t log_parabola_fit_ele_pos_bkg(){
  //
  Int_t nn = 1000;
  Double_t e_min = 0.1;       // GeV
  Double_t e_max = 1000000.0; // GeV
  //
  TGraphErrors *gr_ele_pos_bkg = new TGraphErrors();
  gr_ele_pos_bkg->SetNameTitle("gr_ele_pos_bkg","gr_ele_pos_bkg");
  get_TAUP2023_data(gr_ele_pos_bkg,"./data/TAUP2023_ZhenCao_e+e-.csv");
  //
  //Fit
  const Int_t npar = 6;
  //const Int_t npar = 3;
  ...
  //
  //
  inParameters[0] = -1.86681e-09;
  inParameters[1] =  3.52963e+03;
  inParameters[2] =  2.08620e+00;
  inParameters[3] =  2.33684e-08;
  inParameters[4] =  1.80334e+03;
  inParameters[5] =  3.06795e+00;
  //
  //
  ...
  gr_ele_pos_bkg->Fit("f_log_parabola","","",e_min, e_max);
  //
  //
  ...
  //
  gr_ele_pos_bkg->SetMarkerStyle(20);
  gr_ele_pos_bkg->SetMarkerColor(kBlack);
  gr_ele_pos_bkg->SetLineWidth(2);
  gr_ele_pos_bkg->SetLineColor(kBlack);
  //
  mg->Add(gr_ele_pos_bkg);
  //mg->Add(gr_tev_f);
  //
  mg->SetTitle("p0/(e/p1)^(p2) + p3/(e/p4)^(p5)");
  mg->Draw("APL");
  //
  /*
