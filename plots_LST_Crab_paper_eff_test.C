...
Int_t plots_LST_Crab_paper_eff_test(){
  ...
  cout<<get_crab_rate(gr_trg_effArea,        50.0, 1000.0)<<endl
      <<get_crab_rate(gr_trg_effArea_AdvCam, 50.0, 1000.0)<<endl;
  cout<<get_crab_rate(gr_trg_effArea,        20.0, 300.0)<<endl
      <<get_crab_rate(gr_trg_effArea_AdvCam, 20.0, 300.0)<<endl;
  //
  //
  ...
  TGraphErrors *gr_gamma_rates_trg_PMT_vs_AdvCam_ratio = new TGraphErrors();
  Int_t np_gr_trg_LST;
  ...
  leg06->Draw();
  //
  get_gr_ratio<TGraphErrors>( gr_gamma_rates_trg_LST_AdvCam, gr_gamma_rates_trg_LST, gr_gamma_rates_trg_PMT_vs_AdvCam_ratio, 70, 0.006, 0.3);
  //
  //
  //
  //  
...
void get_gr_ratio( T *gr_num, T *gr_den, T *gr_r, Int_t np, Double_t eMin_TeV, Double_t eMax_TeV){
  ...
  for(Int_t i = 1; i< np;i++){
    ...
    log10_eTeV_L = log10_eTeV_min + (log10_eTeV_max-log10_eTeV_min)/(np-1)*(i-1);
    eTeV = TMath::Power(10.0,(log10_eTeV_R + log10_eTeV_L)/2.0);
    val=gr_num->Eval(eTeV)/gr_den->Eval(eTeV);
    npoints_tot = gr_r->GetN();
    gr_r->SetPoint(npoints_tot,eTeV,val);
    if (typeid(T) == typeid(TGraphErrors))
      gr_r->SetPointError(npoints_tot,0.0,0.0);
  }
  //  
}
