...
Int_t plots_int(){
  ...
  get_flux_p_PDG(gr_flux_p_PDG, "../CTA/cosmique_gamma_hadron_generator/flux_P.csv");
  //     
  //
  //535
  ...
  //
  gr_flux_p_PDG->SetLineColor(kRed+2);
  gr_flux_p_PDG->SetLineWidth(3.0);
  //
  ...
  //mg->GetXaxis()->SetRangeUser(1.0e-2,400);
  ...
  leg->AddEntry(gr_flux_p_PDG, "BESS (PDG) (~35 km)","l");
...
void load_data_file_solar_prot(TString fileInName, TGraph *gr_tot, TGraph *gr_diff){
  ...
  if(fileIn.is_open()){
    ...
    while(fileIn>>ekin>>ftot>>fdiff){
      etot = ekin/1000.0 + 1.0;
      gr_tot->SetPoint(gr_tot->GetN(),etot*1000,ftot);
      if(fdiff>0.0)
	gr_diff->SetPoint(gr_diff->GetN(),etot*1000,fdiff);
    }
    fileIn.close();
  }
  else{
...
void load_data_file(TString fileInName, TGraph *gr_tot, TGraph *gr_diff, Double_t &hMin, Double_t &hMax){
  ...
  if(fileIn.is_open()){
    ...
    while(fileIn>>ekin>>ftot>>fdiff){
      etot = ekin/1000.0 + 1.0;
      gr_tot->SetPoint(gr_tot->GetN(),etot*1000.0,ftot*10000/TMath::Pi()/2.0);
      if(fdiff>0.0)
	gr_diff->SetPoint(gr_diff->GetN(),etot*1000.0,fdiff*10000/TMath::Pi()/2.0);
    }
    fileIn.close();
  }
  else{
...
void get_flux_p_PDG(TGraph *gr, TString name_dat_file){
  ...
  while(indata>>Ekin>>F){
    //cout<<E<<endl;
    if((Ekin+1)<5.5)
      gr->SetPoint(gr->GetN(),(Ekin+1)*1000.0,F);
  }
  indata.close();
}
